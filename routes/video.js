...
router.post("/process", authenticateJWT, async (req, res) => {
  try {
    ...
    if (!video_url && !file_path) {
      return res
        .status(400)
        .json({ error: "Please enter a video URL or upload a file." });
    }
    if (!style || !["crop", "blur", "custom"].includes(style)) {
      return res
        .status(400)
        .json({ error: "Please select a valid style: crop, blur, or custom." });
    }
    ...
    if (video_url) {
      try {
        ...
        return res
          .status(400)
          .json({ error: "Please enter a valid video URL." });
      }
    }
    ...
    if (video_url) {
      ...
      if (!platform) {
        return res.status(400).json({
          error:
            "This URL is not supported. Please use a YouTube, Facebook, Instagram, Vimeo, TikTok, Rumble, Loom, or Dropbox link.",
        });
      }
...
router.get("/status/:videoId", authenticateJWT, async (req, res) => {
  try {
    ...
    return res
      .status(500)
      .json({ error: "Failed to check status.", message: err.message || null });
  }
});
...
router.get("/history", authenticateJWT, async (req, res) => {
  try {
    ...
    return res
      .status(500)
      .json({ error: "Internal server error.", message: err.message || null });
  }
});
...
router.get("/results/:videoId", authenticateJWT, async (req, res) => {
  try {
    ...
    if (vError || !video)
      return res
        .status(404)
        .json({ error: "Video not found.", message: vError?.message || null });
